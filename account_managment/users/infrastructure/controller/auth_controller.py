...
from account_managment.users.infrastructure.utils.generate_token import generate_token
...
async def get_user_with_access_token(payload: Annotated[Payload, Depends(AuthAccessToken)]):
    return await get_user_from_payload(payload)


async def get_user_with_refresh_token(payload: Annotated[Payload, Depends(AuthRefreshToken)]):
    return await get_user_from_payload(payload)
...
async def login(user_signin: UserSigninDto, response: Response) -> Any:
    try:
        ...
        payload_access_token = Payload(sub=user_found.email, exp=expires_access_token, id=str(user_found.id))

        payload_refresh_token = Payload(sub=user_found.email, exp=expires_refresh_token, id=str(user_found.id))

        access_token = JwtUtil.encode(payload_access_token)
        refresh__token = JwtUtil.encode(payload_refresh_token)

        response.set_cookie("access_token", access_token, httponly=True, expires=expires_access_token)
        response.set_cookie(
            "refresh_token",
            refresh__token,
            httponly=True,
...
async def refresh_token(user: Annotated[Users, Depends(get_user_with_refresh_token)], response: Response):
    expires_access_token = EXPIRES_ACCESS_TOKEN
    payload_access_token = Payload(sub=user.email, exp=expires_access_token, id=str(user.id))
